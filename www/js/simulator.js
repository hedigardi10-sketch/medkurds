...
function loadNode() {
    const scenario = scenarios[currentSimIndex];
    ...
    if (node.isTerminal) {
        ...
        feedbackDiv.innerHTML = "<p>" + (node.success ? "سەرکەوتوو بوویت!" : "نەخۆشەکە گیانی لەدەستدا.") + "</p>";
        choicesDiv.appendChild(feedbackDiv);
        
        if (node.success) {
            const nextBtn = document.createElement('button');
            nextBtn.className = 'sim-btn next-btn';
            nextBtn.innerText = "سێناریۆی داهاتوو ➡️";
            ...
            retryBtn.innerText = "دووبارەکردنەوە 🔄";
...
function handleSimDeathByTime() {
    clearSimTimers();
    document.getElementById('sim-title').innerText = "نەخۆشەکە گیانی لەدەست دا 💀";
    document.getElementById('sim-desc').innerHTML = "بەداخەوە کاتت زۆر بەفیڕۆدا یان بڕیارەکانت زۆر لاواز بوون. لە فریاکەوتندا چرکەکان ژیان ڕزگار دەکەن.";
    
    updateVitalsDisplay({ hr: 0, bp: "0/0", spo2: 0 });
    
    const choicesDiv = document.getElementById('sim-choices');
    choicesDiv.innerHTML = '<button class="sim-btn retry-btn" onclick="startScenario(currentSimIndex)">دووبارەکردنەوەی ئەم حاڵەتە 🔄</button>';
}

function showSimEnd(success) {
    document.getElementById('sim-title').innerText = "پیرۆزە! 🎉";
    document.getElementById('sim-desc').innerHTML = "تۆ سەرکەوتووانە هەموو نەخۆشەکانت ڕزگار کرد. سەلماندت کە پزیشک/پەرستارێکی زۆر زیرەکیت لە کاتی فریاگوزاریدا!";
    updateVitalsDisplay({ hr: 80, bp: "120/80", spo2: 99 });
    
    const choicesDiv = document.getElementById('sim-choices');
    choicesDiv.innerHTML = '<button class="sim-btn" onclick="closeSimulator()">داخستنی سیمولەیتەر</button>';
}
